...
func NewWriteBatcher(duration time.Duration) *WriteBatcher {
	return &WriteBatcher{
		duration: duration,
...
// WriteBatcher is a struct that wraps a concurrent sync.Map
...
func (b *WriteBatcher) Load(key interface{}) (value interface{}, ok bool) {
...
func (b *WriteBatcher) Store(key, value interface{}) {
	// prevent the timer from firing as we're manipulating it now
	b.cancelUnfiredTimer()
	// store the key and the value as requested
	log.Tracef("WriteBatcher: Storing key %v and value %q, reset the timer.", key, value)
...
func (b *WriteBatcher) Close() {
	log.Trace("WriteBatcher: Closing the batch channel")
	close(b.flushCh)
}
...
func (b *WriteBatcher) ProcessBatch(fn func(key, val interface{}) bool) bool {
	if _, ok := <-b.flushCh; !ok {
		// channel is closed
		return false
	}
	log.Trace("WriteBatcher: Received a flush for the batch. Dispatching it now.")
	b.syncMap.Range(fn)
	*b.syncMap = sync.Map{}
	return true
}

func (b *WriteBatcher) cancelUnfiredTimer() {
	// If the timer already exists; stop it
	if b.timer != nil {
		log.Tracef("WriteBatcher: Cancelled timer")
		b.timer.Stop()
		b.timer = nil
	}
}

func (b *WriteBatcher) dispatchAfterTimeout() {
	b.timer = time.AfterFunc(b.duration, func() {
		log.Tracef("WriteBatcher: Dispatching a batch job")
		b.flushCh <- struct{}{}
	})
}
